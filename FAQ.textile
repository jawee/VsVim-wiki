...
* <a href="#features">What features are supported?</a>
* <a href="#settings">What settings are supported?</a>
...
* <a href="#vimrc">How can I disable autoload of my vimrc file?</a>
* <a href="#backspace">Why can't I backspace in insert mode?</a>
...
* <a href="#building">How do I build it myself?</a>
* <a href="#keyboard">Why doesn't the Ctrl+[ key (or some other key) work?</a>
* <a href="#selection">Text selection doesn't work right.  Can this be fixed?</a>
* <a href="#options">How do I go back to resolve conflicting keybindings?</a>
* <a href="#install-latest">How can I install a build with the current sources?</a>
...
VsVim implements the Vim <code>backspace</code> setting.  The default value for this setting doesn't allow backspace at the start of insert mode.  If you have a <code>vimrc</code> file VsVim will respect this setting and behave as Vim would.  To change this behavior add the following to your <code>vimrc</code> file 

<pre><code>set backspace=indent,eol,start</pre></code>

<a name="vimrc"></a>
*How can I disable autoload of my vimrc file?*
...
VsVim can be temporary disabled by the key sequence Ctrl+Shift+F12 and re-enabled later by the same key sequence.  While disabled VsVim will not interfere with any keyboard or selection allowing you to use Visual Studio as if VsVim wasn't installed.
...
<a name="keyboard"></a>
*Why doesn't the Ctrl+[ key (or some other key) work?*

VsVim tries hard to make as many keys that you that you are accustomed to using in vim available.  If for some reason a key is not working, try [[Troubleshooting the Keyboard|Troubleshooting-the-Keyboard]].

<a name="selection"></a>
*Text selection doesn't work right.  Can this be fixed?*

If you are not accustomed to vim's visual mode but still want vi-like keybindings, you can enable select mode which is much more like the select you are familiar with in Visual Studio. See [[VsVim Nonstandard Behavior|VsVim-Nonstandard-Behavior#wiki-ctrl-x-ctrl-c-ctrl-v]], in particular <pre><code>behave mswin</code></pre>.


<a name="options"></a>
*How do I go back to resolve conflicting keybindings?*

Open VsVim options using the button to the right of the commandline (at the bottom right of your editor window)

<a name="install-latest"></a>
*How can I install a build with the current sources?*

Every commit in VsVim will upload a new build to [the Open VSIX Gallery](http://vsixgallery.com).  If there is a bug fix, patch which isn't on the official Microsoft gallery yet you can just install the latest VsVim build from here.  The instructions for using this as a VSIX source are documented here.

- http://vsixgallery.com/guide/feed/
